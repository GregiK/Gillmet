...
export async function POST(req: NextRequest) {
  ...
  const upstream = await fetch(`${BASE_URL}/api/auth/reset-password`, {
    ...
    body: JSON.stringify({ token: body.token, haslo: body.haslo }),
    cache: "no-store",
  });
  const data = await upstream.json().catch(() => ({}));

  if (!upstream.ok || !data.ok) {
    return NextResponse.json({ error: data.error || "Nie udalo sie zresetowac hasla." }, { status: upstream.status || 400 });
  }
